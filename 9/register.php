    ...
    try {
        ...
        if ($flag == false) {
            ...
            if ($result) {
                ...
                //登録情報書き込み用のSQL作成
                $stmt = $pdo->prepare("INSERT INTO users (id, name, mail, password, date_added, date_updated, admin_flg, life_flg)
                VALUES( NULL, :name, :userid, :password, sysdate(), sysdate(), 0, 0)");
...
<html lang="ja">
    ...
    <body>
        <header class="headerGlobal">
            ...
            <div class="userMenu">
                管理用：
                <div class="button" id="loginButton"><a href="./login.php">Login</a></div>
            </div>
        </header>
        <h1>ユーザー登録</h1>
        <main>
            <form id="login" class="questionnaire" method="post" action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>">
                ...
                <fieldset>
                    ...
                    <p class="required"><span class="itemName">E-mail</span>
                        <span class="itemBody"><input class="answer required" type="email" name="userid" placeholder="例）●●●●@●●●●.●●●" size="20" title="E-mailアドレスを入力してください。" required pattern="^\S+@\S+\.\S+$"></span>
                    </p>
                    <p class="required"><span class="itemName">パスワード</span>
                        <span class="itemBody"><input class="answer required" type="password" name="password" placeholder="●●●●●●●●" size="20" title="パスワードを入力してください。"></span>
                    </p>
                </fieldset>
                <!--ラジオボタン、セレクトボックスは面倒-->
